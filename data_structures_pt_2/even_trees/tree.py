...
class SimpleTree:
    ...
    def GetNodes(self, node, nodesList):
        if len(node.Children) == 0:
            return
        nodesList.extend(reversed(node.Children))
        for node in node.Children:
            self.GetNodes(node, nodesList)

    def EvenTrees(self):
        nodesList = [self.Root]
        result = []

        self.GetNodes(self.Root, nodesList)
